...
/*   Updated: 2025/02/17 01:12:09 by mel-houd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Config::CheckLocationPath(const RouteConf& Location)
{
	if (Location.GetLocationPath().empty())
		throw std::runtime_error("no Location in Route");
}
...
void	Config::CheckMethods(const RouteConf& Location)
{
	...
	for (it = Location.GetMethods().cbegin(); it != Location.GetMethods().cend(); ++it)
	{
		if (*it != "GET" && *it != "POST" && *it != "DELETE" && *it != "PUT")
			throw std::runtime_error("invalid HTPP method: " + *it);
	}
}